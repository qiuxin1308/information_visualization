importsVis = function(_parentElement){
	this.parentElement = _parentElement;
	this.initVis();
}

importsVis.prototype.initVis = function(){
	var vis = this;
	vis.barVis();
	//vis.asterPlotVis(dataImports);
};

/* function to draw bar chart */
importsVis.prototype.barVis = function(){
	var vis = this;
	...
	vis.titleSvg = d3.select("#trade_title")
	                 ...
	                 .text("Wine Import In Seven Countries (in billions)");
	...
	vis.barTitle = vis.barSvg.append("text")
	                     ...
	                     .text("Wine Import Between 2013 and 2017");
	...
	vis.z = d3.scaleOrdinal()
	          .range(["#c2c2d6","#bf80ff"]); //"#763730", "#268079"//"#c2c2d6", "#bf80ff" //"#EBE0E4", "#BF739C"

	var keys = ["2013", "2017"];
	vis.x0.domain(imTrendData.map(function(d){ return d.Country;}));
	vis.x1.domain(keys).rangeRound([0, vis.x0.bandwidth()]);
	vis.y.domain([0, d3.max(imTrendData, function(d){
	     ...
	     .on("click", function(d){
	     	if (d.key === "2013") {
	     		...
	     		for(var i = 0; i < dataImports.length; i++){
	     			if (d.country === dataImports[i].importer && dataImports[i].year === "2013") {
	     				countryImports.push({"country": dataImports[i].country,"import":dataImports[i].import,"color":dataImports[i].color});
	     			}
	     		}
	     		//console.log(countryImports);
	     		$("#leftarrow").hide();
	     		vis.pieVis(countryImports,d.country,d.key);
	     	}else if (d.key === "2017") {
	     		var countryImports = [];
	     		//console.log(dataImports);
	     		for(var i = 0; i < dataImports.length; i++){
	     			if (d.country === dataImports[i].importer && dataImports[i].year === "2017") {
	     				countryImports.push({"country": dataImports[i].country,"import":dataImports[i].import,"color":dataImports[i].color});
	     			}
	     		}
	     		//console.log(countryImports);
...
importsVis.prototype.pieVis = function(data,selectedCountry,curYear){
	var vis = this;
	$("#asterView").empty();
	$("#asterLegend").empty();
	//console.log(data);
	...
	vis.asterTitle = vis.asterSvg.append("text")
	                     		...
	                     		.attr("x",55)
	                     		.attr("y",13)
	                     		.text("Countries where the wine in "+selectedCountry+" comes from in "+curYear)
	                     		.append("tspan")
	                     		.attr("x",145)
	                     		.attr("y",40)
	                     		//.style("font-famliy", "Arial")
	                     		.style("font-size", "14px")
	                     		.text("("+f(total)+" billions total for wine import in "+selectedCountry+")");
	d3.select("#asterSvg")
	  .datum(dataArr)
	  .call(vis.donut);
...
importsVis.prototype.donutChart = function(){
	var width,
        height,
...
importsVis.prototype.asterPlotVis = function(data){
	var vis = this;
	var total = 0;
	//console.log(wineTradeData);
	vis.asterWidth = 680;
	...
	vis.tip = d3.tip()
	            ...
	            .offset([0,0])
	            .html(function(d){
	            	return d.data.country + ": <span style='color:orangered'>" +f(d.data.import)+ "</span>";
	            });

	vis.arc = d3.arc()
	            .innerRadius(vis.innerRadius)
	            .outerRadius(function(d){
	            	return (vis.radius - vis.innerRadius) * (d.data.import / 8.0) + vis.innerRadius;
	            });
	...
	vis.asterTitle = vis.asterSvg.append("text")
	                     		...
	                     		.attr("x",140)
	                     		.attr("y",20)
	                     		.text("Wine Import in 2017")
	                     		.append("tspan")
	                     		.attr("x",125)
	                     		.attr("y",40)
	                     		.style("font-famliy", "Arial")
	                     		.style("font-size", "10px")
	                     		.text("("+f(total)+" billions total among 7 countries)");
